...
var i = {
  ...
  emoticonFormat: function (e) {
    ...
    e = e.replace(new RegExp('(\\[.+?\\])(?!' + MM_EMOTICON_WEB + ')', 'g'),
            function (e, a) {
              return t.getEmoticonByText(a) || e
            }).replace(new RegExp('&lt;(.+?)&gt;(?!' + MM_EMOTICON_WEB + ')', 'g'),
                function (e, a) {
                  return t.getEmoticonByText('<' + a + '>') || e
                })
  ...
  getEmoticonByText: function (e) {
    var t = this.QQFaceMap[e.replace(/\[|\]/g, '')]
  ...
  getEmoticonByCode: function (txt) {
    let re

    for (let key in this.EmojiCodeMap) {
      re = new RegExp(this.EmojiCodeMap[key], 'gi')
      txt = txt.replace(re, `<span title="" type="emoji" class="emoji emoji${key}"></span>`)
    }
    return txt
  },
  getCodeByText: function (txt) {
    let t = this.QQFaceMap[txt]
    return t
  },
  formatSendContent: function (txt) {
    let tmp = ''
    let result = ''

    for (let char of txt) {
      tmp = char.codePointAt(0).toString(16)
      tmp = this.EmojiCodeMap[tmp]
