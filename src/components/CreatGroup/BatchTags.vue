    ...
    <MaskLayer :isShowBox="isShowBox">
      <template slot="MaskBox">
        <div class="propBox">
          ...
          <div class="step_box" v-if="steps == 1">
            <p>选择客户</p>
          </div>
          <span v-if="steps == 2">
          ...
          <div class="footer1 cf" v-if="steps != 3">
            ...
            <span v-if="steps == 2">已选：{{tagNumber}}个标签</span>
...
export default {
  ...
  async created () {
    ...
    // //  memberNoGm: userInfo.memberNoGm,
    //   merchantNo: userInfo.merchantNo,
    //   noWxGm: userInfo.noWxGm
    // })
  },
  ...
  methods: {
   ...
   selectTags (type) {
      this.$http.post('/api/member/findPmLabelListByMerchantNo.do',{
        ...
        if(res.data.result){
          this.tagsData = res.data.returnObject
        }
    ...
    sureBtn () {
      ...
      // console.log(this.tagsCodes)
      if (this.tagNumber && this.tagsNames && this.tagsCodes) {

        this.$http.post('/api/imh5/member/addTags.do',{
          tags: this.tagsCodes.join(','),
          names: this.tagsNames.join(','),
          memberNos: this.checkboxList.join(','),
          merchantNo: this.userInfo.memberNoMerchant,
          type: this.tagsType
        }).then(res=>{
          if(res.data.result){
